import { id } from './id';
import RoyCustomPanel from './RoyCustomPanel';
...
export default {
  ...
  preRegistration: ({ servicesManager, commandsManager, configuration = {} }) => {
    ...
    uiNotificationService.show({
      ...
      message: 'Custom Roy Extension is now active',
      type: 'success'
    });
  },
  /**
  ...
  getToolbarModule: ({ servicesManager, commandsManager, extensionManager }) => {
     return {
      definitions: {
        RoyToolButton: {
          id: 'RoyToolButton',
          icon: 'ellipse-circle', // Using existing OHIF icon
          label: 'Roy Button',
          type: 'command',
          commandName: 'showRoyAlert'
        }
      },
      ...
      definitions: {
        showRoyAlert: {
          commandFn: () => {
            const { uiNotificationService } = servicesManager.services;
            uiNotificationService.show({
              title: 'Extension Works!',
              message: 'This is from your custom extension',
              type: 'info'
            });
          }
        }
